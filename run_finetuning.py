...
def run_finetuning(config: configure_finetuning.FinetuningConfig):
  ...
  pred_label_dir = os.path.join(config.data_dir, 'models', config.model_name, 'finetuning_models')
    ...
    if config.do_eval:
      if config.write_test_outputs and trial <= config.n_writes_test:
        ...
        for task in tasks:
          ...
          elif task.name in ["cmedqq", "qyilu", "medrel", "dner", "cmedic", "cmedtc", "cmedqaner", 'cehrner', 'cmedir', 'cmedqa', 'cmedqnli']:
              scorer = model_runner.evaluate_task(task, "dev", False)
              preds, labels, logits = scorer._get_preds_and_labels()
              txt_path = os.path.join(pred_label_dir, task.name + '_' + 'dev' + '_' + str(trial))
              utils.log("Write prediction and labels", txt_path)
              out = open(txt_path, 'w')
              for pred, label in zip(preds, labels):
                out.write('\t' + str(label) + '\t' + str(pred) + '\n')
          else:
        ...
        for task in tasks:
          ...
          if task.name in ["cola", "mrpc", "mnli", "sst", "rte", "qnli", "qqp",
            ...
            if config.num_trials > 1:
              ...
              scorer = model_runner.evaluate_task(task, 'eval', False)
              preds, labels, logits = scorer._get_preds_and_labels()
              txt_path = os.path.join(pred_label_dir, task.name + '_eval_' + str(trial))
              utils.log("Write prediction and labels", txt_path)
              out = open(txt_path, 'w')
              for pred, label, logits in zip(preds, labels, logits):
                out.write('\t' + str(label) + '\t' + str(pred) + '\t' + str(logits.tolist()) + '\n')
              out.close()
          else:
            utils.log("Skipping task", task.name,
                      "- writing predictions is not supported for this task")
