...
class SingleOutputTask(task.Task):
  ...
  def _load_glue_json_line(self, lines, skip_first_line=False, eid_offset=0, swap=False):
    examples = []
    j = 0
    for (i, line) in enumerate(lines):
      try:
        ...
        for doc in content["documents"]:
          eid = j - (1 if skip_first_line else 0) + eid_offset
          j += 1
          reply = doc["title"]
          label = doc["label"]
          text_a = tokenization.convert_to_unicode(query)
          text_b = tokenization.convert_to_unicode(reply)
          label = tokenization.convert_to_unicode(label)
          examples.append(
                  InputExample(eid=eid, task_name=self.name, text_a=text_a, text_b=text_b, label=label))
      except Exception as ex:
        utils.log("Error constructing example from line", i,
                  "for task", self.name + ":", ex)
        utils.log("Input causing the error:", line)
...
class cMedIR(RegressionTask):
  """Semantic Textual Similarity."""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(cMedIR, self).__init__(config, "cmedir", tokenizer, 0.0, 4.0)

  def _create_examples(self, lines, split):
    examples = []
    if split == "test":
      examples += self._load_glue(lines, split, 0, 1, None, False)
    else:
      examples += self._load_glue(lines, split, 0, 1, 2, False)
    if self.config.double_unordered and split == "train":
      examples += self._load_glue(
          lines, split, 0, 1, 2, True, len(examples), False)
    return examples

  #def get_examples(self, split):
  #  return self._create_examples(read_tsv(
  #      os.path.join(self.config.raw_data_dir(self.name), split + ".json"),
  #      max_lines=100 if self.config.debug else None), split)

  #def _create_examples(self, lines, split):
  #  examples = []
  #  examples += self._load_glue_json_line(lines, True)
  #  return examples
#
...
class cMedQA(ClassificationTask):
  """cMedQA."""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(cMedQA, self).__init__(config, "cmedqa", tokenizer,
                               ["0", "1"])
  ...
  def _create_examples(self, lines, split):
    return self._load_glue(lines, split, 1, 2, 0)

  def get_scorer(self):
    return classification_metrics.F1Scorer()

class cMedQQ(ClassificationTask):
  """cMedQA."""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(cMedQQ, self).__init__(config, "cmedqq", tokenizer,
                               ["0", "1"])
...
class cMedQNLI(ClassificationTask):
  """cMedQNLI."""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(cMedQNLI, self).__init__(config, "cmedqnli", tokenizer,
                               ["0", "1"])
...
class QYILU(ClassificationTask):
  """QYILU."""
  def get_scorer(self):
    return classification_metrics.F1Scorer()

class cMedIC(ClassificationTask):
  """cMedIC."""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(cMedIC, self).__init__(config, "cmedic", tokenizer,
                               ["0", "1", "2"])
...
class QYILU(ClassificationTask):
  ...
  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(QYILU, self).__init__(config, "qyilu", tokenizer,
                               ['101','102','103','104','105','106','107','108','109','199','201','202','203','204','205','206','207','208','209','210','211','299'])
  ...
  def _create_examples(self, lines, split):
    return self._load_glue(lines, split, 0, None, 1)

  def get_scorer(self):
    return classification_metrics.AccuracyScorer()

class MedRel(ClassificationTask):
  """medrel."""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(MedRel, self).__init__(config, "medrel", tokenizer,
                               ["0", "1"])
  ...
  def _create_examples(self, lines, split):
    return self._load_glue(lines, split, 0, 1, 2)
