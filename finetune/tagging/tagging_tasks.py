...
class TaggingTask(task.Task):
  ...
  def _get_label_mapping(self, provided_split=None, provided_sentences=None):
    ...
    train_tags = set()
    for split in ["train", "dev", "test", "eval"]:
...
class dNer(TaggingTask):
  """Text dner."""

  def __init__(self, config, tokenizer):
    super(dNer, self).__init__(config, "dner", tokenizer, False)

class cMedQANer(TaggingTask):
  """Text cMedQANer."""

  def __init__(self, config, tokenizer):
    super(cMedQANer, self).__init__(config, "cmedqaner", tokenizer, False)

class cEHRNER(TaggingTask):
  """Text cEHRNER."""

  def __init__(self, config, tokenizer):
    super(cEHRNER, self).__init__(config, "cehrner", tokenizer, False)
