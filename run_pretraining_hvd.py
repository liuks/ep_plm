...
from model import optimization_hvd
...
class PretrainingModel(object):
    ...
    def __init__(self, config: configure_pretraining.PretrainingConfig,
        ...
        if config.pap_task:
            # must be same as the config used in the tf-records building stage
            tokenizer = tokenization.FullTokenizer(
                vocab_file=config.vocab_file,
                do_lower_case=config.do_lower_case,
                cws_input=False)  # no Chinese word segments input for PAP task
            self.phrase = tokenization.Phrase(config.pap_phrase_file)
            self.v_input_ids, self.v_input_mask = self.phrase.preload(
                tokenizer, config.pap_attr_value_max_length, return_value='tf')

            self.eval_metrics = None
            self.total_loss = tf.cond(
                tf.equal(tf.reduce_sum(features['task_id']), 0),
                lambda: self.task_lm(config, pretrain_data.rename_keys(features, task_id=0), is_training),
                lambda: self.task_pap(config, pretrain_data.rename_keys(features, task_id=1), is_training))
            self.task_id_sum = tf.reduce_sum(features['task_id'])
        else:
            self.eval_metrics = None
            self.total_loss = self.task_lm(config, features, is_training)

    def task_lm(self, config: configure_pretraining.PretrainingConfig,
                features, is_training):

        # Mask the input
        masked_inputs = pretrain_helpers.mask(
            config, pretrain_data.features_to_inputs(features, task_id=0), config.mask_prob)
        # Generator
        embedding_size = (
    ...
    def task_pap(self, config: configure_pretraining.PretrainingConfig,
                 features, is_training):
        ...
        inputs = pretrain_data.features_to_inputs(features, task_id=1)

        title = self._build_transformer(
            pretrain_data.BertInputs(
                input_ids=inputs.t_input_ids,
                input_mask=inputs.t_input_mask,
                segment_ids=inputs.t_segment_ids),
            is_training, reuse=True, embedding_size=embedding_size)

        attr_name = self._build_transformer(
            pretrain_data.BertInputs(
                input_ids=inputs.n_input_ids,
                input_mask=inputs.n_input_mask,
                segment_ids=inputs.n_segment_ids),
            is_training, reuse=True, embedding_size=embedding_size)

        head_relation = tf.concat(
            [title.get_pooled_output(), attr_name.get_pooled_output()],
            axis=1, name="title_and_attr_name")

        v_neg_words_ids = pretrain_helpers.pap_negative_samples(config, inputs.v_word_ids, len(self.phrase))
        phrase_input_ids, phrase_input_mask, phrase_labels = pretrain_helpers.pap_value_word_tokens(
            config, inputs.v_word_ids, v_neg_words_ids, self.v_input_ids, self.v_input_mask)

        B, PN, L = modeling.get_shape_list(phrase_input_ids, expected_rank=3)
        v_input_ids = tf.reshape(phrase_input_ids, [-1, L])
        v_input_mask = tf.reshape(phrase_input_mask, [-1, L])

        flat_tail = self._build_transformer(
            pretrain_data.BertInputs(
                input_ids=v_input_ids,
                input_mask=v_input_mask,
                segment_ids=tf.zeros_like(v_input_ids)),
            is_training, reuse=True, embedding_size=embedding_size)

        tail = tf.cast(tf.expand_dims(phrase_labels, -1), tf.float32) * tf.reshape(
            flat_tail.get_pooled_output(), [B, PN, -1])

        with tf.variable_scope("pap_predictions"):
            # B * H
            head_relation_hidden = tf.layers.dense(
                head_relation,
            ...
            dist = tf.reduce_sum(tf.math.abs(
                tf.expand_dims(head_relation_hidden, axis=1) - tail_hidden), axis=-1)

            score = - tf.math.log_sigmoid(
                (config.pap_loss_gamma - dist) * tf.cast(phrase_labels, tf.float32))

            pos_sum = tf.reduce_sum(tf.cast(phrase_labels > 0, tf.float32), axis=-1, keepdims=True)
            neg_sum = tf.reduce_sum(tf.cast(phrase_labels < 0, tf.float32), axis=-1, keepdims=True)

            score = (tf.where(phrase_labels > 0, score / (pos_sum + 1E-6), tf.zeros_like(score)) +
                     tf.where(phrase_labels < 0, score / (neg_sum + 1E-6), tf.zeros_like(score)))

            loss = tf.reduce_sum(score) / config.train_batch_size
        #
        # self.eval_metrics = (lambda: 0, [])
        # return tf.reduce_sum(tf.cast(features["t_input_ids"], dtype=tf.float32))

        eval_fn_keys = ["pap_loss"]
        eval_fn_values = {"pap_loss": loss}

        def metric_fn(*args):
            """Computes the loss  of the model."""
            d = {k: arg for k, arg in zip(eval_fn_keys, args)}
            metrics = {"pap_loss": tf.metrics.mean(d["pap_loss"])}
            return metrics
    ...
    def _build_transformer(self, inputs: Union[pretrain_data.Inputs, pretrain_data.BertInputs],
                           is_training, bert_config=None, name="electra", reuse=False, **kwargs):
...
def model_fn_builder(config: configure_pretraining.PretrainingConfig):
    ...
    def model_fn(features, labels, mode, params):
        ...
        if config.use_tpu:
            ...
            to_log = {'loss': model.total_loss}
            if config.pap_task:
                to_log['task_id_sum'] = model.task_id_sum
...
def train_or_eval(config: configure_pretraining.PretrainingConfig):
    ...
    if hvd.rank() != 0:
        ...
        # config.model_dir = os.path.join(config.model_dir, str(hvd.rank()))
    config.num_train_steps //= hvd.size()
    config.num_warmup_steps //= hvd.size()
    ...
    proto.gpu_options.visible_device_list = str(hvd.local_rank())
...
def train_one_step(config: configure_pretraining.PretrainingConfig):
    ...
    if config.pap_task:
        ...
        features = train_input_fn({"batch_size": 4})
        iterator = features.make_initializable_iterator()
        one_element = iterator.get_next()
        model = PretrainingModel(config, one_element, True)
        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            sess.run(iterator.initializer)
            for i in range(13):
                f, m = sess.run([one_element, model.total_loss])
                print('> iter:', i)
                # print(f)
                print(f['task_id'])
                print(m)
...
def main():
    ...
    config = configure_pretraining.PretrainingConfig(args.model_name, args.data_dir, **hparams)
    print("config", config)
