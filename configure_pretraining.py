...
class PretrainingConfig(object):
    ...
    def __init__(self, model_name, data_dir, **kwargs):
        ...
        self.pretrain_data_dir = ""

        self.cws_input = False
        ...
        # PAP task
        self.pap_task = False
        self.pap_dataset_prob = 0.01
        self.pap_pretrain_tfrecords = os.path.join(
            data_dir, "pap_pretrain_tfrecords/pretrain_data.tfrecord*")
        self.pap_phrase_file = os.path.join(data_dir, "phrase.txt")

        self.pap_title_max_length = 128
        self.pap_attr_name_max_length = 10
        self.pap_attr_value_max_length = 20
        self.pap_attr_value_max_words = 20
        self.pap_negative_ratio = 2
        self.pap_loss_gamma = 4.0  # Margin of the KE objective

        # update defaults with passed-in hyperparameters
        self.update(kwargs)

        if self.pretrain_data_dir != "":
            self.pretrain_tfrecords = os.path.join(data_dir, self.pretrain_data_dir, "pretrain_tfrecords/pretrain_data.tfrecord*")
    ...
    def __str__(self):
        a = ['{}: {}'.format(k, self.__dict__[k]) for k in sorted(self.__dict__.keys())]
        return '\n'.join(a)
